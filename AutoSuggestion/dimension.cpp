 ...
 * @param wordGrams
 */
dimension::dimension(unordered_map<string, vector<string>> word_ngrams) {
 ...
 */
unsigned long dimension::count_words() {
    return (unsigned long) this->word_ngrams.size();
}
...
void dimension::initialize_gram_indices() {
    ...
    /**
     * 1. Generate the gram and their association to the words, this helps us to
     * find all the words to make vector comparisons with optimized way.
     */
    build_gram_words();

    /**
     * 2. Build Gram Indices.
     */
    build_gram_indices();

    /**
     * 3. Accumulate ngram_weight for all the words entries using tf-idf.
     */
    build_weighted_word_ngrams();

    /**
    * 4. Build the word and their associated vector.
    */
    build_word_vector();
    //display_in_console(this->words_vector, "All words vector: ");
}

/**
 * Builds weights for all the word grams.
 */
void dimension::build_weighted_word_ngrams() {
    this->word_ngram_scores = weighted_word_ngrams(this->word_ngrams, this->gram_words, this->count_words());
}
 ...
 * Build Gram Indices.
 */
void dimension::build_gram_indices() {
    map<string, vector<string>> ordered_grams(this->gram_words.begin(), this->gram_words.end());
    int index = 0;
...
void dimension::build_word_vector() {
    auto word_weights_iter = this->word_ngram_scores.begin();
    for (; word_weights_iter != this->word_ngram_scores.end(); word_weights_iter++) {
        string word = word_weights_iter->first;
        vector<pair<index, score>> w_vector = generate_vector(word_weights_iter->second);
        this->words_vector.insert(make_pair(word, w_vector));
    }
}
...
vector<pair<string, double>> dimension::suggest(string partial_word) {
    ...
    //display_in_console(grams, "NGrams of a input word: ");

    vector<pair<index, score>> grams_vector = generate_input_vector(grams);
    //display_in_console(grams_vector, "A Vector : ");
    ...
    ranked_scores = top_scored_words(ordered_scores);
...
vector<pair<string, double>> dimension::filter_sort_by_score(const unordered_map<string, double> &word_scores) {
    vector<pair<string, score>> sorted_scores;
 ...
 */
vector<pair<int, double>> dimension::generate_input_vector(const vector<string> &grams) {
    vector<pair<string, score>> input_weights = weighted_grams(grams, this->gram_words, this->count_words());
    return generate_vector(input_weights);
}

/**
 * Generate vector for list of grams.
 */
vector<pair<int, double>> dimension::generate_vector(const vector<pair<string, double>> &grams) {
    vector<pair<index, double>> vector_entry;
    for (auto ng_iter = grams.begin(); ng_iter != grams.end(); ng_iter++) {
        string gram = ng_iter->first;
        if (this->gram_index.find(gram) != this->gram_index.end()) {
            vector_entry.emplace_back(make_pair(this->gram_index[gram], ng_iter->second));
        }
    }
    return vector_entry;
}
 ...
 */
unordered_map<string, vector<pair<int, double>>> dimension::acquire_vectors(const set<string> &candidate_words) {
...
dimension::compute_similarity(const unordered_map<string, vector<pair<int, double>>> &candidates,
